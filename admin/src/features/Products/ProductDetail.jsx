...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProductDetail = async () => {
      try {
        setLoading(true);
        const response = await httpGet(`/product/admin-product/${id}`, {
          showMessage: false,
        });
  ...
  const getSellerStatusTag = (status) => {
    const statusConfig = {
      ...
      UNLIST: { color: "orange", text: "Ẩn" },
      DRAFT: { color: "blue", text: "Bản nháp" },
      DELETED: { color: "red", text: "Đã xóa (Seller)" },
    };
    ...
    <Card>
      ...
      <Descriptions bordered column={2} style={{ marginBottom: "24px" }}>
        <Descriptions.Item label="Tên cửa hàng">
          <Text strong>{product.shop_id?.shop?.shopName || "N/A"}</Text>
        </Descriptions.Item>
        <Descriptions.Item label="Username">
          {product.shop_id?.username}
        </Descriptions.Item>
        ...
        <Descriptions.Item label="Trạng thái xác thực">
          <Tag
            color={
              product.shop_id?.shop?.verificationStatus === "approved"
                ? "green"
                : "orange"
            }
          >
            {product.shop_id?.shop?.verificationStatus || "N/A"}
          </Tag>
        </Descriptions.Item>
      </Descriptions>
      <Divider />
